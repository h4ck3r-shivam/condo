...
class TelegramAuthRoutes {
    ...
    async completeAuth (req, res, next) {
        try {
            ...
            if (!session || session.payload.checks.state !== state) {
            ...
            if (session.payload.checks.nonce !== decodedToken.nonce) {
                console.error(session.payload.checks.nonce, decodedToken.nonce)
                return res.end()
            }
            ...
            await redisClient.set(
                getRedisSessionKey(verifiedUniqueKey),
    ...
    async getAuthToken (req, res, next) {
        try {
            ...
            console.error('UNIQUE_KEY', uniqueKey)
